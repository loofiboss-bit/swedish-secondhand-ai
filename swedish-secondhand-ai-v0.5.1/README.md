...
- Configurable Gemini or local Ollama item analysis with deterministic heuristic fallback.
...
- Gemini API keys are user-provided and currently stored locally in application settings.
- Ollama uses a user-configured local OpenAI-compatible base URL and model.
- Provider secrets will move behind the Electron security boundary in the dedicated v0.6 milestone.
