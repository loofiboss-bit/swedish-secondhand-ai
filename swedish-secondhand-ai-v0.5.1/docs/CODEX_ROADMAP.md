...
```text
v0.5.1  Development baseline (internal gate)
...
# v0.5.1 — Development Baseline

## Milestone objective

Restore a green, reproducible development baseline before provider architecture work begins.

### In scope

- Make `docs/CODEX_ROADMAP.md` the canonical implementation roadmap.
- Keep project-specific `AGENTS.md` governance outside external config synchronization.
- Restore lint and formatting validation without formatting editor-local or externally synchronized files.
- Preserve settings persistence while removing effect-driven local-state synchronization.
- Separate read-only application E2E tests from documentation screenshot generation.
- Bring README, user guide, and changelog in sync with current Gemini and Ollama behavior.

### Out of scope

- No provider contracts or adapter migration.
- No secret migration or IPC changes.
- No valuation, listing, domain, or persisted-schema changes.
- No version tag or public release.

### Definition of done

- `npm run validate` passes from a clean checkout.
- `npm run test:e2e` passes without modifying tracked files.
- Documentation screenshots remain available through an explicit command.
- Current user-visible behavior and persisted data remain unchanged.

### Rollback point

This milestone can be reverted as a normal code and documentation change because it does not alter
persisted data or runtime provider behavior.
...
Implement v0.6 milestones in this dependency and security order:
...
The Electron boundary is intentionally completed before new cloud providers are introduced, so new
provider secrets never enter renderer persistence.
...
```text
Read AGENTS.md, docs/CODEX_ROADMAP.md, .ai/ROADMAP.md, README.md,
