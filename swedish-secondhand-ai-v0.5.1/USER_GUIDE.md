...
Gemini requires a user-provided API key. Ollama can run against a locally configured endpoint without
a cloud API key. If the selected provider is unavailable, heuristic analysis still works with lower
confidence. Tradera comparables require a valid Tradera API key.
...
### Ollama analysis falls back to heuristics

- Verify that Ollama is running.
- Verify the configured base URL and model name.
- Use a vision-capable model when analyzing images.
