...
describe('SettingsPanel', () => {
  ...
  it('hydrates persisted input values when settings change', async () => {
    render(<SettingsPanel />);
    ...
    act(() => {
      useSettingsStore.setState({
        settings: {
          ...baseSettings,
          geminiApiKey: 'hydrated-gemini-key',
          traderaApiKey: 'hydrated-tradera-key',
          ollamaBaseUrl: 'http://localhost:22434/v1',
          ollamaModel: 'hydrated-vision-model',
        },
      });
    });

    expect(screen.getByPlaceholderText('AIza...')).toHaveValue('hydrated-gemini-key');
    expect(screen.getByPlaceholderText('Tradera API key')).toHaveValue('hydrated-tradera-key');
    expect(screen.getByPlaceholderText('http://localhost:11434/v1')).toHaveValue(
      'http://localhost:22434/v1',
    );
    expect(screen.getByPlaceholderText('llava')).toHaveValue('hydrated-vision-model');
  });
