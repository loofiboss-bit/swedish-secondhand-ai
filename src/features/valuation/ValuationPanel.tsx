...
export function ValuationPanel() {
  ...
  const { activeProject, setActivePriceDecision } = useProjectStore();
  ...
  const [ownPrice, setOwnPrice] = useState(
    activeProject?.priceDecision.kind === 'user_entered'
      ? String(activeProject.priceDecision.amountSek)
      : '',
  );
  ...
  return (
    <SectionCard title={t('estimate')}>
      ...
      <section className="price-decision" aria-labelledby="price-decision-title">
        <p className="eyebrow">{t('priceStep')}</p>
        <h3 id="price-decision-title">{t('choosePricePath')}</h3>
        <p>{t('choosePricePathIntro')}</p>
        <div className="price-paths">
          <button
            type="button"
            className={activeProject?.priceDecision.kind === 'user_entered' ? 'is-selected' : ''}
            onClick={() => document.getElementById('own-price-input')?.focus()}
          >
            {t('useOwnPrice')}
          </button>
          <button
            type="button"
            className={activeProject?.priceDecision.kind === 'evidence_based' ? 'is-selected' : ''}
            disabled={!valuation || valuation.status === 'insufficient-evidence'}
            onClick={() => {
              if (valuation && valuation.status !== 'insufficient-evidence') {
                void setActivePriceDecision({ kind: 'evidence_based', valuation });
              }
            }}
          >
            {t('useEvidencePrice')}
          ...
          <button
            type="button"
            className={activeProject?.priceDecision.kind === 'unset' ? 'is-selected' : ''}
            onClick={() => void setActivePriceDecision({ kind: 'unset' })}
          >
            {t('decidePriceLater')}
          </button>
        </div>
        <form
          className="own-price-form"
          onSubmit={(event) => {
            event.preventDefault();
            const amountSek = Number(ownPrice);
            if (Number.isFinite(amountSek) && amountSek > 0) {
              void setActivePriceDecision({ kind: 'user_entered', amountSek });
            }
          }}
        >
          <label className="field" htmlFor="own-price-input">
          ...
          <button type="submit">{t('confirmOwnPrice')}</button>
        </form>
        {activeProject?.priceDecision.kind === 'user_entered' && (
          <p className="own-price-badge" role="status">
            {t('yourPrice')}: {activeProject.priceDecision.amountSek} SEK
          </p>
        )}
      </section>

      <details className="advanced-pricing">
        <summary>{t('evidencePriceAdvanced')}</summary>
        ...
        <div className="comparable-filters">
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setPriceKindFilter(event.target.value as 'all' | MarketPriceKind)
              }
            >
